...
const [status,setStatus] = useState('')
    ...
    const handleMyQuery=async(e)=>{
        ...
        console.log(localStorage.getItem('userName'),message);
        await axios.post("http://localhost:8888/api/createquery",{role:'customer',userName:localStorage.getItem('userName'),message:message}).then(resp =>{
                setStatus(resp.data)
    ...
    let groupedByName = []
      groupedByName.push([allQueries.filter((eachQuery)=>eachQuery.userName === localStorage.getItem('userName'))])
